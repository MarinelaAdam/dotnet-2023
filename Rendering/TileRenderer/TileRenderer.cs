...
public static class TileRenderer
{
    ...
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        if (feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Highway && MapFeature.HighwayTypes.Any(v => p.Value.StartsWith(v))))
        {
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Water || p.Key == MapFeatureData.StringReplacer.Water_point || p.Key == MapFeatureData.StringReplacer.Waterway ) && feature.Type != GeometryType.Point)
        {
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Railway))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Natural && featureType == GeometryType.Polygon))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Boundary && string.Equals(p.Value, "forest", StringComparison.OrdinalIgnoreCase)))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Landuse && (string.Equals(p.Value, "forest", StringComparison.OrdinalIgnoreCase) || string.Equals(p.Value, "orchard", StringComparison.OrdinalIgnoreCase))))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
            baseShape = geoFeature;
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == MapFeatureData.StringReplacer.Landuse && (string.Equals(p.Value, "residential", StringComparison.OrdinalIgnoreCase) || string.Equals(p.Value, "cemetery", StringComparison.OrdinalIgnoreCase) || string.Equals(p.Value, "industrial", StringComparison.OrdinalIgnoreCase) || string.Equals(p.Value, "commercial", StringComparison.OrdinalIgnoreCase) ||
                                                        string.Equals(p.Value, "square", StringComparison.OrdinalIgnoreCase) || string.Equals(p.Value, "construction", StringComparison.OrdinalIgnoreCase) || string.Equals(p.Value, "military", StringComparison.OrdinalIgnoreCase) || string.Equals(p.Value, "quarry", StringComparison.OrdinalIgnoreCase) ||
                                                        string.Equals(p.Value, "brownfield", StringComparison.OrdinalIgnoreCase))))
        {
            var coordinates = feature.Coordinates;
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == MapFeatureData.StringReplacer.Landuse && (p.Value.StartsWith("farm") || p.Value.StartsWith("meadow") || p.Value.StartsWith("grass") || p.Value.StartsWith("greenfield") ||
                                                        p.Value.StartsWith("recreation_ground") || p.Value.StartsWith("winter_sports") || p.Value.StartsWith("allotments"))))
        {
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                 ...
                 feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Landuse && (p.Value.StartsWith("reservoir") || p.Value.StartsWith("basin"))))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Building))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Leisure))
        { 
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == MapFeatureData.StringReplacer.Amenity))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
